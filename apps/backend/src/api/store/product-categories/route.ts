...
function parseFields(fieldsStr: string | undefined): string[] {
  if (!fieldsStr)
    return ['id', 'handle', 'name', 'parent_category_id', 'rank', 'metadata']
  return fieldsStr
    .split(',')
    .map((f) => f.trim())
    .filter(Boolean)
...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  const hasChildren =
    fieldsStr?.includes('category_children') ||
    fieldsStr === '*category_children,metadata'
    ...
    if (hasChildren) {
      for (const cat of result) {
        if (cat.category_children?.length) {
          cat.category_children = translateCategories(
