...
export default async function syncPostexStatusJob(container: MedusaContainer) {
  ...
  console.log(
    `🔹 [POSTEX SYNC] Found ${activeShipments.length} shipments to check`
  )

  for (const shipment of activeShipments) {
    try {
      const statusCode = await postexService.getParcelStatus(
        shipment.postex_parcel_id
      )

      if (!statusCode) {
        console.warn(
          `⚠️ [POSTEX SYNC] Could not get status for parcel ${shipment.postex_parcel_id}`
        )
        continue
      }
      ...
      if (newStatus !== shipment.status) {
        ...
        console.log(
          `✅ [POSTEX SYNC] Updated shipment ${shipment.id} status: ${shipment.status} → ${newStatus}`
        )
      }
    } catch (error) {
      console.error(
        `❌ [POSTEX SYNC] Error processing shipment ${shipment.id}:`,
        error
      )
    }
  }
