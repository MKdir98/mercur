/**
 * Deletes documents older than KIBANA_LOG_RETENTION_DAYS (default 2) from the
 ...
 *
 * Cron example (daily at 03:00, keep last 2 days):
 *   0 3 * * * cd /path/to/mercur/apps/backend && KIBANA_ES_NODE=http://localhost:9200 KIBANA_LOG_RETENTION_DAYS=2 npx ts-node src/scripts/prune-kibana-logs.ts >> /var/log/prune-kibana-logs.log 2>&1
 */
...
async function main() {
  const retentionDays = Number(process.env.KIBANA_LOG_RETENTION_DAYS) || 2
